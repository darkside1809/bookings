...
func TestRepository_Reseravation(t *testing.T) {
	...
	// test case where reservation in not is session (reset everithing) 
	req, _ = http.NewRequest("GET", "/make-reservation", nil)
	ctx = getCtx(req)
	req = req.WithContext(ctx)
	rr = httptest.NewRecorder()
...
func TestRepository_PostReseravation(t *testing.T) {
	reqBody := "start_date=2050-01-01"
	reqBody = fmt.Sprintf("%s&%s", reqBody, "end_date=2050-01-02")
	reqBody = fmt.Sprintf("%s&%s", reqBody, "first_name=John")
	reqBody = fmt.Sprintf("%s&%s", reqBody, "last_name=Smith")
	reqBody = fmt.Sprintf("%s&%s", reqBody, "email=jsmith@gmail.com")
	reqBody = fmt.Sprintf("%s&%s", reqBody, "phone=9992990129")
	...
	if rr.Code != http.StatusSeeOther {
		t.Errorf("PostReseravation handler returned wrong response code: got %d, wanted %d", rr.Code, http.StatusSeeOther)
	}
